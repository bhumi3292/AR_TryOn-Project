...
try:
    ...
    dr = None
    _nvdiffrast_import_error = e
    print("⚠️ nvdiffrast import failed — GPU rasterizer unavailable:", e)
    print("If you need full InstantMesh functionality, install nvdiffrast or run on Linux/WSL where nvdiffrast binaries are available.")
...
def interpolate(attr, rast, attr_idx, rast_db=None):
    if dr is None:
        raise RuntimeError("nvdiffrast is not available. Install nvdiffrast or run in an environment where it is supported.")
...
def xatlas_uvmap(ctx, mesh_v, mesh_pos_idx, resolution):
    ...
    # rasterize
    if dr is None:
        raise RuntimeError("nvdiffrast is not available. xatlas_uvmap requires nvdiffrast for rasterization.")
